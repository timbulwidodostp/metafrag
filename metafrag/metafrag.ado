*! 2.0.1 Ariel Linden 30nov2021 // fixed word wrapping of description of results
	...
	* ]                    
	...
	qui {
		...
		*********************************************************	
		...
		if `"`forest'`forest2'"' != "" { 
			meta forestplot, nullrefline `eform' ///
				...
				columnopts(_a _c, title("Events")) columnopts(_b _d, title("Nonevents")) `forest2'
		...
		* Add red or blue color to event values that were subtracted or added, respectively		
		...
		di _n
		if "`frag'" == "`cnt_fi'" {
			di as txt "   The pooled treatment effect turns statistically nonsignificant "
			di as txt "   after " %1.0f `frag' " event-status modifications" 
		}
		if "`frag'" != "`cnt_fi'" {
			di as txt "   The pooled treatment effect turns statistically nonsignificant "
			di as txt "   after " %1.0f `frag' " iterations accounting for " %1.0f `cnt_fi' "" 
			di as txt "   event-status modifications"
		}
